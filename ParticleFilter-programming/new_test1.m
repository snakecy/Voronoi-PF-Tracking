...
for k=1:LL0
    dist10(k)=sqrt((dis_sensor1(k,1)-x0(1,1))^2+(dis_sensor1(k,2)-x0(1,2))^2);
end
[ss,sensor_leader1(1)]=min(dist10);
sensor_range=10;%sensor range 感知半径
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
hold on;
for t=2:T
    v_mes(t,1)=sum(w(t,:).*velocity(t,:,1));
    v_mes(t,2)=sum(w(t,:).*velocity(t,:,2));
    for j=1:N
        error2(t,j)=w(t,j)*((velocity(t,j,1)-v0(t,1))^2+(velocity(t,j,2)-v0(t,2))^2);
    end
    error2_(t)=sqrt(sum(error1(t,:)));
end
plot(1:T,error2_(:))
% plot(2:T,v0(2:T,1),'b-')
% hold on;
% plot(2:T,v_mes(2:T,1),'r:');
% xlabel('x','fontsize',15);
% ylabel('y','fontsize',15);
